...
public final class FileSystemMatchers {
	private static boolean isFileSystemLocation(Object o) {
		try {
			Class<?> FileSystemLocation = Class.forName("org.gradle.api.file.FileSystemLocation");
			return FileSystemLocation.isInstance(o);
	...
	public static <T> Matcher<T> aFile(Matcher<? super File> matcher) {
		return new FeatureMatcher<T, File>(matcher, "a file", "the file") {
			...
			protected File featureValueOf(T actual) {
				if (isFileSystemLocation(actual)) {
	...
	public static Matcher<File> absolutePath(String expectedPath) {
		return withAbsolutePath(equalTo(expectedPath));
	}
	...
	public static Matcher<Object> anEmptyDirectory() {
		return allOf(anExistingDirectory(), aFile(new TypeSafeDiagnosingMatcher<File>() {
			@Override
			protected boolean matchesSafely(File file, Description description) {
				return descendants(file).isEmpty();
			}
	...
	public static Matcher<Object> hasDescendants(Matcher<? super DescendantFile>... descendantMatchers) {
		return hasDescendants(Arrays.asList(descendantMatchers));
	}
